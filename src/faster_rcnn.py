...
class FasterRCNN(nn.Module):
    ...
    def filter_boxes_by_scores_and_size(self, cls_logits, pred_boxes):
        ...
        indxes = torch.where(torch.gt(scores, self.pred_score_thresh))[0]
        boxes = boxes[indxes]
    ...
    def forward(
        ...
        if self.training:
            ...
            box_sep_val = torch.max(boxes) + 1
            separated_boxes = boxes + (cls_idxes * box_sep_val)[:, None]
            keep = nms(separated_boxes, scores, config.output_nms_within_class)
